...
//for each parity bit, it uses the parity mask to find out which bits are required position are set to 1, then the function counts the number of bits set to 1 and stores it in 1. If the number of bits is odd (checked using mod 2), then a parity bit is added in the specificied parity positions (1, 2, 4, 8)
